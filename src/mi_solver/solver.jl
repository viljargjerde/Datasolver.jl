...
# function get_initialization_s(f, node_vector, cross_section_area, num_ele, constrained_dofs, numQuadPts = 2)
# 	target_f = create_initialization_target_function(f, node_vector, cross_section_area, num_ele, constrained_dofs, numQuadPts)
# 	s = zeros(num_ele)
# 	problem = NonlinearProblem(target_f, s)
...
# end
# function create_initialization_target_function(f, node_vector, cross_section_area, num_ele, constrained_dofs, numQuadPts = 2)
# 	function target_function(s, p)
# 		quad_pts, quad_weights = GaussLegendreQuadRule(numQuadPts = numQuadPts)
# 		dims = size(node_vector[1])[1]
# 		N_func, dN_func = constructBasisFunctionMatrixLinearLagrange(dims)
# 		# R_matrix = constructBasisFunctionMatrixConstantFuncs(evalPts = quad_pts)
...
# 				equilibrium[active_dofs_u] += N_matrix' * (quad_weight * J4int * f) -
# 											  (dN_matrix') * (integration_factor) * (dPhih * sh)

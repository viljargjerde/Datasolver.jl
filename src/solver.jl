...
function _init_expr_array(T::Type, n::Int)
	if T <: JuMP.AbstractJuMPScalar
		return [JuMP.QuadExpr() for _ in 1:n]
	else
		return zeros(T, n)
	end
end
...
function equilibrium_eq(uhat, sbar, problem::Dataproblem)
	...
	# equilibrium = zeros(eltype(uhat), length(problem.num_node) * dims)
	# equilibrium = zeros(length(problem.num_node) * dims)
	equilibrium = _init_expr_array(eltype(uhat), problem.num_node * dims)
...
function compatibility_eq(uhat, ebar, problem::Dataproblem)
	...
	# compatibility = zeros(eltype(uhat), problem.num_ele)
	# compatibility = zeros(problem.num_ele)
	# compatibility = [QuadExpr() for _ in 1:(problem.num_ele)]
	compatibility = _init_expr_array(eltype(uhat), problem.num_ele)
...
function directSolverNonLinearBarA(;
	...
	if verbose
		total_NRiter = sum([sum(n) for n in results.NRiter])
		println("Computation took in total $(sum(results.ADMiter)) ADM iters and $(total_NRiter) NR iters.")
	end
	return results
end

...
function NLP_solver(
	...
	# free_dofs = setdiff(1:n*dims, fixed_dofs)
	...
	@variable(model, uhat[1:n*dims])

	equilibrium = equilibrium_eq(uhat, sbar, problem)
	compatibility = compatibility_eq(uhat, ebar, problem)
	# equilibrium_eq = zeros(QuadExpr, n*dims)
	...
	@constraint(model, equilibrium .== 0)
	@constraint(model, compatibility .== 0)

	#########
	@variable(model, choosen_ES[1:m, 1:length(dataset)], Bin)
	# @variable(model, 0 <= choosen_ES[1:m, 1:length(dataset)] <= 1)
	...
	if termination_status(model) == MOI.OPTIMAL || termination_status(model) == MOI.TIME_LIMIT || termination_status(model) == MOI.OTHER_LIMIT
		...
		s_values = value.(sbar)
		u_values = value.(uhat)
		equilibrium_values = value.(equilibrium)
		compatibility_values = value.(compatibility)

		E_values = value.(E)
		S_values = value.(S)
	else
		println("The model did not solve to optimality. Termination status: ", termination_status(model))
		# Print the model
		print(model)
...
function _integrateCostfunction_NLP_L1(abs_ediff::AbstractArray, abs_sdiff::AbstractArray, costFunc_constant::Float64, problem::Dataproblem)

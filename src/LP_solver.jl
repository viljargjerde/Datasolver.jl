...
function nonlin_LP_solver(node_vector, data, cross_section_area, bar_distF, fixed_dofs; alpha = 1.0, verbose = false)
	...
	# first_term =
	# 	(
	# 		(cross_section_area * quad_weight * J4ints[i]) * sbar[i] * (1.0 + alpha * (dN_func(quad_pt)/J4derivs[i]*uhat[i:i+1])[1]) * (dN_func(quad_pt)/J4derivs[i]*lambdahat[i:i+1])[1] for i in 1:m,
	# 		(quad_pt, quad_weight) in zip(quad_pts, quad_weights)
	# 	)
	# second_term =
	# 	((quad_weight * J4ints[i]) * (cross_section_area * (dN_func(quad_pt)/J4derivs[i]*lambdahat[i:i+1])[1] * (N_func(quad_pt)*bar_distF)[1]) for i in 1:m, (quad_pt, quad_weight) in zip(quad_pts, quad_weights))
	# third_term = (
	# 	(cross_section_area * quad_weight * J4ints[i]) * mubar[i] * ((dN_func(quad_pt)/J4derivs[i]*uhat[i:i+1])[1] + alpha / 2 * (dN_func(quad_pt)/J4derivs[i]*uhat[i:i+1])[1] * (dN_func(quad_pt)/J4derivs[i]*uhat[i:i+1])[1] - ebar[i]) for i in 1:m,
	# 	(quad_pt, quad_weight) in zip(quad_pts, quad_weights)
	# )

	first_term = ((cross_section_area * quad_weight * J4ints[i]) * sbar[i] * (1.0 + alpha * (dN_func(quad_pt)/J4derivs[i]*uhat[i:i+1])[1]) for i in 1:m,
				  (quad_pt, quad_weight) in zip(quad_pts, quad_weights))



	second_term = ((quad_weight * J4ints[i]) * (N_func(quad_pt)*bar_distF)[1] for i in 1:m, (quad_pt, quad_weight) in zip(quad_pts, quad_weights))

	third_term = (
		(cross_section_area * quad_weight * J4ints[i]) * ((dN_func(quad_pt)/J4derivs[i]*uhat[i:i+1])[1] + alpha / 2 * (dN_func(quad_pt)/J4derivs[i]*uhat[i:i+1])[1]^2 - ebar[i]) for i in 1:m,
		(quad_pt, quad_weight) in zip(quad_pts, quad_weights)
	)


	@constraint(model, sum(collect(first_term) - collect(second_term), dims = 2) .== 0)
	@constraint(model, sum(collect(third_term), dims = 2) .== 0)
	# @constraint(model, sum(collect(first_term) - collect(second_term) + collect(third_term), dims = 2) .== 0)
	# @constraint(model, third_term == 0)

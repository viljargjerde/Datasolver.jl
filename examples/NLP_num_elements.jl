...
update_tex_command(all_results_file, "NLPElementsPowerlawNull", format(FormatExpr("W(M) \\propto M^{{{:.2f}}}"), b_null))
...
update_tex_command(all_results_file, "NLPElementsPowerlawRand", format(FormatExpr("W(M) \\propto M^{{{:.2f}}}"), b_rand))

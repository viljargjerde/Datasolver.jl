...
plot!(xs_fitted, f1.(xs_fitted), label = L"W(\mathtt{D}) = %$(latex_sci(a_null)) \ \mathtt{D}^{%$(round(b_null, sigdigits = 2))}", linestyle = :dash)
...
plot!(xs_fitted, f2.(xs_fitted), label = L"W(\mathtt{D}) = %$(latex_sci(a_null)) \ \mathtt{D}^{%$(round(b_null, sigdigits = 2))}", linestyle = :dash)

savefig(replace(results_file, "results.json" => "lineplot.tex"))
uncomment_pgfplotsset_blocks(dirname(results_file))

update_tex_command(all_results_file, "NLPElementsPowerlawNull", format(FormatExpr("W(\\mathtt{{M}}) \\propto \\mathtt{{M}}^{{{:.2f}}}"), b_null))
update_tex_command(all_results_file, "NLPElementsPowerlawNullB", format(FormatExpr("{:.2g}"), b_null))

update_tex_command(all_results_file, "NLPElementsPowerlawRand", format(FormatExpr("W(\\mathtt{{M}}) \\propto \\mathtt{{M}}^{{{:.2f}}}"), b_rand))
